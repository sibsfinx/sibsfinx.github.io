/**
 * Cloudflare Worker: Markdown content negotiation for the portfolio.
 * Deploy with: npx wrangler deploy
 * Requires assets binding to the static site (see wrangler.toml).
 */
...
export default {
  async fetch(request: Request, env: { ASSETS: Fetcher }): Promise<Response> {
    ...
    if (chosen === "text/markdown") {
      ...
      const notFound = await env.ASSETS.fetch(new Request(new URL("/404.md", url).toString()));
      const body =
        notFound.status === 200
          ? notFound.body
    ...
    if (res.headers.get("content-type")?.includes("text/html")) {
      const linkValue = `<${markdownPath(url.pathname)}>; rel="alternate"; type="text/markdown"`;
